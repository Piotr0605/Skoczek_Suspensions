...
import { Clock, CheckCircle, Package, AlertTriangle, Settings, RefreshCw } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    // Simulate fetching from a real database via API
    try {
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-end md:items-center border-b border-gray-200 pb-6 gap-4">
        ...
        <button 
          ...
          className="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-gray-500 hover:text-skoczek-primary transition"
        >
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden">
        ...
        <div className="p-6 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
           <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-white border border-gray-200 text-skoczek-primary flex items-center justify-center font-black text-2xl rounded-full shadow-sm">
                 {user.name.charAt(0)}
              </div>
              <div>
                 <h3 className="text-black font-bold text-lg">{user.name}</h3>
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 text-gray-500 text-xs font-bold uppercase tracking-wider">
                <tr>
                  ...
                  <th className="p-5 border-b border-gray-200 hidden md:table-cell">Metoda</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {requests.map((req) => (
                  <tr key={req.id} className="hover:bg-gray-50 transition group">
                    ...
                    <td className="p-5">
                      <span className={`inline-flex items-center gap-2 px-3 py-1.5 border rounded-full text-xs font-bold uppercase tracking-wide ${statusStyles[req.status]}`}>
                        {statusIcons[req.status]}
                        {req.status}
                      </span>
                    </td>
                    <td className="p-5 text-gray-500 hidden md:table-cell text-sm font-medium">
