...
const EMAILJS_TEMPLATE_ID: string = 'WPISZ_TU_SWOJ_TEMPLATE_ID'; // np. template_abc123

// 3. Public Key: Wejdź w Account -> API Keys -> Public Key i wklej go tutaj
const EMAILJS_PUBLIC_KEY: string = 'WPISZ_TU_SWOJ_PUBLIC_KEY'; // np. user_XaB12...
...
const ServiceForm: React.FC<ServiceFormProps> = ({ user, onSuccess, onRequestLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (EMAILJS_TEMPLATE_ID !== 'WPISZ_TU_SWOJ_TEMPLATE_ID' && EMAILJS_PUBLIC_KEY !== 'WPISZ_TU_SWOJ_PUBLIC_KEY') {
          try {
            await emailjs.send(
                EMAILJS_SERVICE_ID,
                EMAILJS_TEMPLATE_ID,
